...
from sqlalchemy import Boolean, Enum, ForeignKey, Numeric, String, Text, text
...
class Company(TenantMixin, BaseModel):
    ...
    tags: Mapped[list | None] = mapped_column(JSONB, default=list, server_default=text("'[]'::jsonb"))
...
class CompanyUser(TenantMixin, BaseModel):
...
class Contact(TenantMixin, BaseModel):
...
class UserAddress(TenantMixin, BaseModel):
