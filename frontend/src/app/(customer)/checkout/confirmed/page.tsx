...
export default function CheckoutConfirmedPage() {
  ...
  return (
    <div style={{ background: "var(--ui-paper)", padding: "64px 24px", display: "flex", flexDirection: "column", alignItems: "center", minHeight: "60vh", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ maxWidth: "600px", width: "100%" }}>

        {/* Success checkmark */}
        <div style={{ marginBottom: "20px", textAlign: "center" }}>
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
            <circle cx="24" cy="24" r="24" fill="var(--brand-primary, var(--ui-ink))" opacity="0.1"/>
            <path d="M14 24l7 7 13-13" stroke="var(--brand-primary, var(--ui-ink))" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>

        <h1 className="ui-h1" style={{ fontSize: "34px", marginBottom: "12px" }}>
          Order Confirmed
        </h1>
        <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "14px", color: "var(--ui-muted)", marginBottom: "8px" }}>
          {confirmedOrderNumber}
        </p>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "16px", color: "var(--ui-muted)", marginBottom: "36px", lineHeight: 1.6 }}>
          Your order has been placed. You will receive a confirmation email shortly.
        </p>

        {/* Details box */}
        <div style={{ border: "1px solid var(--ui-line)", background: "#FFFFFF", padding: "24px", marginBottom: "28px" }}>
          {[
            ...
            <div key={row.label} style={{ display: "flex", gap: "12px", padding: "10px 0", borderBottom: i < arr.length - 1 ? "1px solid var(--ui-line)" : "none" }}>
              <dt style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "var(--ui-muted)", minWidth: "200px", margin: 0 }}>{row.label}</dt>
              <dd style={{ fontFamily: (row as { mono?: boolean }).mono ? "'IBM Plex Mono', monospace" : "'DM Sans', sans-serif", fontSize: (row as { mono?: boolean }).mono ? "13px" : "14px", color: "var(--ui-ink)", margin: 0, fontWeight: 500 }}>{row.value}</dd>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
          <Link
            ...
            style={{ display: "inline-block", padding: "12px 24px", border: "1px solid var(--brand-primary, var(--ui-ink))", color: "var(--brand-primary, var(--ui-ink))", fontFamily: "'DM Sans', sans-serif", fontSize: "14px", fontWeight: 500, textDecoration: "none", transition: "all .15s" }}
          >
            Track Your Order →
          </Link>
          <Link
            href="/products"
            style={{ display: "inline-block", padding: "12px 24px", background: "var(--brand-primary, var(--ui-ink))", color: "#fff", fontFamily: "'DM Sans', sans-serif", fontSize: "14px", fontWeight: 500, textDecoration: "none", transition: "opacity .15s" }}
          >
            Continue Shopping →
          </Link>
        </div>
