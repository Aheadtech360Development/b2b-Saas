  ...
  if (children.length === 0) {
    return (
      <Link href={item.href || "#"} style={navLinkStyle}
        ...
        onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.color = "var(--brand-primary, #1C3557)"; }}>
        {item.label}
      </Link>
    );
  }
        ...
        <div style={{ position: "absolute", top: "100%", left: 0, background: "#fff", border: "1px solid #E2E2DE", padding: "6px", minWidth: "210px", boxShadow: "0 8px 24px rgba(0,0,0,.1)", zIndex: 100 }}>
          {children.map((c) => (
            <Link key={c.label + c.href} href={c.href || "#"}
              style={{ display: "block", padding: "9px 12px", color: "var(--brand-primary, #1C3557)", fontSize: "15px", fontWeight: 500, textDecoration: "none", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)", transition: "background .15s" }}
...
export function Header() {
  ...
  const navLinkStyle: React.CSSProperties = {
    color: "var(--brand-primary, #1C3557)",
    fontSize: "18px",
    fontWeight: 500,
    textDecoration: "none",
    letterSpacing: ".01em",
  ...
  const layout = branding.header_layout || "logo_left";
  const showCart = branding.show_cart !== false;

  // ── Header pieces (reused across layouts) ───────────────────────────────────
  const logoNode = (
    <Link href="/" style={{ display: "flex", alignItems: "center", textDecoration: "none", gap: "10px" }}>
      {branding.logo_url ? (
        ...
        <span style={{ fontFamily: "var(--brand-font-heading, 'Fraunces', serif)", fontSize: "24px", fontWeight: 700, color: branding.primary_color, letterSpacing: "-.01em" }}>
          {branding.store_name}
        </span>
      )}
    </Link>
  ...
  const actionsNode = (
    <div style={{ display: "flex", gap: "8px", alignItems: "center", justifyContent: "flex-end" }}>
      {/* Cart (toggleable) */}
      {showCart && (
        <Link href="/cart" style={{ position: "relative", background: "transparent", border: "1px solid #E2E2DE", color: "var(--brand-primary, #1C3557)", padding: "8px 12px", cursor: "pointer", fontSize: "18px", transition: "all .2s", display: "flex", alignItems: "center" }}>
          <ShoppingCartIcon size={18} color="var(--brand-primary, #1C3557)" />
          {cartCount > 0 && (
            <span style={{ position: "absolute", top: "-6px", right: "-6px", background: "var(--brand-primary, #1C3557)", color: "#fff", fontSize: "9px", fontWeight: 700, width: "18px", height: "18px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {cartCount}
            </span>
          )}
        </Link>
      ...
      {/* The storefront is customer-facing: admins see the exact customer view
          (no admin chrome). Only real customers get their account + sign out. */}
      <div className="hidden md:flex" style={{ gap: "10px", alignItems: "center" }}>
        {authed && !isAdminUser ? (
          <>
            <Link href="/account" style={{ fontSize: "15px", color: "var(--brand-primary, #1C3557)", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)", textDecoration: "none" }}>
              {user?.first_name || "My Account"}
            </Link>
            <button
              onClick={handleLogout}
              style={{ background: "transparent", color: "var(--brand-primary, #1C3557)", padding: "8px 16px", fontSize: "15px", border: "1px solid #E2E2DE", cursor: "pointer", fontWeight: 500, transition: "all .2s", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)", borderRadius: "6px" }}
            >
              Sign out
            </button>
          </>
      ...
      <button
        ...
        className="md:hidden"
        style={{ padding: "8px", color: "var(--brand-primary, #1C3557)", background: "transparent", border: "1px solid #E2E2DE", cursor: "pointer" }}
  ...
  const baseInner: React.CSSProperties = { maxWidth: "1500px", margin: "0 auto", padding: "0 32px" };
      ...
      <header style={{ background: "#FFFFFF", borderBottom: "1px solid #E2E2DE", position: "sticky", top: 0, zIndex: 1 }}>
        {layout === "logo_center" ? (
          ...
          <div className="header-inner" style={{ ...baseInner, display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", height: "90px", gap: "16px" }}>
            <div style={{ justifySelf: "start", minWidth: 0 }}>{navNode}</div>
            <div style={{ justifySelf: "center" }}>{logoNode}</div>
            <div style={{ justifySelf: "end" }}>{actionsNode}</div>
          </div>
        ) : layout === "logo_center_below" ? (
          // Logo centered on top, menu row centered below
          <div className="header-inner" style={{ ...baseInner, display: "flex", flexDirection: "column", padding: "14px 32px", gap: "6px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", width: "100%" }}>
              <div />
              <div style={{ justifySelf: "center" }}>{logoNode}</div>
              <div style={{ justifySelf: "end" }}>{actionsNode}</div>
            </div>
            <div style={{ display: "flex", justifyContent: "center" }}>{navNode}</div>
          </div>
        ) : (
          // Default: logo left · menu beside · actions right
          <div className="header-inner" style={{ ...baseInner, display: "flex", alignItems: "center", justifyContent: "space-between", height: "90px", gap: "24px" }}>
            {logoNode}
            {navNode}
            {actionsNode}
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "1px solid #E2E2DE" }}>
              {branding.logo_url ? (
                ...
                <span style={{ fontFamily: "var(--brand-font-heading, 'Fraunces', serif)", fontSize: "18px", fontWeight: 700, color: branding.primary_color }}>{branding.store_name}</span>
              )}
              <button
                onClick={() => setMenuOpen(false)}
            ...
            <div style={{ padding: "16px 24px" }}>
              {(branding.menu_items?.length ? branding.menu_items : [{ href: "/products", label: "Shop All" }]).map((item) => (
                <div key={item.label + item.href}>
                  <Link href={item.href || "#"} onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "12px 0", color: "var(--brand-primary, #1C3557)", fontSize: "14px", fontWeight: 600, textDecoration: "none", borderBottom: "1px solid #E2E2DE", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
                    {item.label}
                  </Link>
                  {(item.children ?? []).map((c) => (
                    <Link key={c.label + c.href} href={c.href || "#"} onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "10px 0 10px 16px", color: "#6B6B6B", fontSize: "13px", fontWeight: 500, textDecoration: "none", borderBottom: "1px solid #F0EFEA", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
                      {c.label}
                    </Link>
                  ))}
                </div>
              ...
              {authed && !isAdminUser && (
                <>
                  <Link href="/quick-order" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "12px 0", color: "var(--brand-primary, #1C3557)", fontSize: "14px", fontWeight: 500, textDecoration: "none", borderBottom: "1px solid #E2E2DE", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
                    Quick Order
                  </Link>
                  <Link href="/account" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "12px 0", color: "var(--brand-primary, #1C3557)", fontSize: "14px", fontWeight: 500, textDecoration: "none", borderBottom: "1px solid #E2E2DE", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
                    My Account
                  </Link>
                  <Link href="/cart" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "12px 0", color: "var(--brand-primary, #1C3557)", fontSize: "14px", fontWeight: 500, textDecoration: "none", borderBottom: "1px solid #E2E2DE", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
              ...
              {authed && !isAdminUser && (
                <button onClick={handleLogout} style={{ marginTop: "12px", display: "block", width: "100%", textAlign: "center", padding: "12px", color: "var(--brand-primary, #1C3557)", fontSize: "18px", fontWeight: 500, background: "transparent", border: "1px solid #E2E2DE", cursor: "pointer", fontFamily: "var(--brand-font-body, 'DM Sans', sans-serif)" }}>
                  Sign Out
                </button>
              )}
            </div>
